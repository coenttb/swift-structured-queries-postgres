...
extension QueryExpression {
    /// Alias for jsonAgg() to maintain compatibility with SQLite's jsonGroupArray
    /// In PostgreSQL, this maps to json_agg()
...
public func jsonbBuildObject(_ pairs: (String, any QueryExpression)...) -> some QueryExpression<Data> {
...
extension QueryExpression {
    ...
    public func filter(where condition: some QueryExpression<Bool>) -> some QueryExpression<QueryValue> {
